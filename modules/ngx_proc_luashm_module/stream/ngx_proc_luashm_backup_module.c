...
typedef struct {
	...
	uint64_t                     expires;
    u_char                       data[1];
} ngx_stream_lua_shdict_node_ext_t;
...
ngx_module_t ngx_proc_stream_luashm_backup_module = {
    NGX_MODULE_V1,
...
static ngx_int_t ngx_proc_luashm_backup_prepare(ngx_cycle_t *cycle)
{
    ...
    pbcf = ngx_proc_get_conf(cycle->conf_ctx, ngx_proc_stream_luashm_backup_module);
	...
	for (; i < pbcf->shdict_names->nelts;i++)
	{
		...
		zone = ngx_stream_lua_find_zone(name->data,name->len);
...
static int ngx_stream_lua_shdict_expire(ngx_stream_lua_shdict_ctx_t *ctx, ngx_uint_t n)
{
    ngx_time_t                  *tp;
    uint64_t                     now;
    ngx_queue_t                 *q;
...
static int ngx_proc_luashm_backup_recover(ngx_stream_lua_shdict_ctx_t *ctx,ngx_stream_lua_shdict_node_ext_t *extsd){
    ngx_stream_lua_shdict_node_t  *sd;
	ngx_rbtree_node_t           *node;
	int                          i, n;
	uint32_t                     hash;
...
static int ngx_proc_luashm_backup_backup(ngx_cycle_t *cycle,ngx_shm_zone_t *zone)
{
    ...
    ngx_queue_t                 	*q, *prev;
    ngx_stream_lua_shdict_node_t  	*sd;
	ngx_stream_lua_shdict_node_ext_t 	*extsd;
    ...
    while (q != ngx_queue_sentinel(&ctx->sh->queue)) {
        ...
        if (sd->expires == 0 || ((int64_t)(sd->expires - now)) > 60000) {
			...
			extsd = (ngx_stream_lua_shdict_node_ext_t*)buff->last;
			...
			buff->last += sizeof(ngx_stream_lua_shdict_node_ext_t)+len;
...
static ngx_int_t ngx_proc_luashm_backup_process_init(ngx_cycle_t *cycle)
{
	...
	ngx_stream_lua_shdict_node_ext_t  *extsd;
	ngx_shm_zone_t              	*zone;
	ngx_stream_lua_shdict_ctx_t   	*sdctx;
	ngx_file_t 		 				file;
	ngx_str_t  						*name;
	ngx_buf_t 						*buff;
	u_char							*shm_file_path;
	...
	for (; i < pbcf->shdict_names->nelts;i++)
	{
		...
		if(n>0){
			...
			while(buff->last - buff->pos >0){
				extsd=(ngx_stream_lua_shdict_node_ext_t*)buff->pos;
				if(extsd->expires == 0 || ((int64_t)(extsd->expires-now) > 60000))
					ngx_proc_luashm_backup_recover(sdctx,extsd);

				buff->pos+=sizeof(ngx_stream_lua_shdict_node_ext_t)+extsd->key_len+extsd->value_len;
			}
		}
		ngx_pfree(pbcf->pool,shm_file_path);
		ngx_pfree(pbcf->pool,buff->start);
