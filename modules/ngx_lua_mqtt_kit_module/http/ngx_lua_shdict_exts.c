...
static int ngx_http_shdict_push_helper(ngx_str_t *zoneName,ngx_str_t *_key, ngx_str_t *_value,int flags);
...
static ngx_inline int ngx_http_lua_shdict_expire(ngx_http_lua_shdict_ctx_t *ctx, ngx_uint_t n)
{
    ngx_time_t                  *tp;
    uint64_t                     now;
    ngx_queue_t                 *q;
...
ngx_lua_ffi_shdict_lpush(u_char *zone,u_char *_key, u_char *_value,ngx_uint_t len)
{
    ...
    return ngx_http_shdict_push_helper(&zoneName,&key,&value, NGX_HTTP_LUA_SHDICT_LEFT);
}
...
ngx_lua_ffi_shdict_rpush(u_char *zone,u_char *_key, u_char *_value,ngx_uint_t len)
{
    ...
    return ngx_http_shdict_push_helper(&zoneName,&key,&value, NGX_HTTP_LUA_SHDICT_RIGHT);
}

static int
ngx_http_shdict_push_helper(ngx_str_t *zoneName,ngx_str_t *_key, ngx_str_t *_value,int flags)
{
    int                              n;
    ngx_str_t                        key;
    uint32_t                         hash;
	...
	zone = ngx_http_lua_find_zone(zoneName->data,zoneName->len);
	if(zone==NULL){
...
ngx_lua_ffi_shdict_lpop(u_char *zone,u_char *_key, ngx_str_t *out)
{
    ...
    return ngx_lua_shdict_pop_helper(&zoneName,&key,out, NGX_HTTP_LUA_SHDICT_LEFT);
}
...
ngx_lua_ffi_shdict_rpop(u_char *zone,u_char *_key, ngx_str_t *out)
{
    ...
    return ngx_lua_shdict_pop_helper(&zoneName,&key,out, NGX_HTTP_LUA_SHDICT_RIGHT);
}
