...
ngx_http_modsecurity_save_headers_out(ngx_http_request_t *r)
{
	...
	if(r->upstream->conf==NULL){

		//ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "ModSecurity: upstream->conf is NULL");

        //return NGX_ERROR;
        r->upstream->conf=upstream->conf;
	}
...
ngx_http_modsecurity_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
	...
	/* TODO: XXX fixed endless loop while Content-Length: 0  */
