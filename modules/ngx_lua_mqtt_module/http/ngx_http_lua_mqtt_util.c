...
int ngx_ffi_mqtt_offset_process(u_char *zoneName,u_char *_data,int d_len){
	...
	msg_id=utility_mqtt_read_int64(p,remaining_length);
	if(msg_id==-1)goto finished;
	p+=8;
	index+=8;
	...
	while(index < remaining_length){
		...
		if(msg_value.len>0){
			...
			store_zone = ngx_http_lua_find_zone(zoneName,ngx_strlen(zoneName));
			if(store_zone!=NULL)
		...
		zone = ngx_http_lua_find_zone(zName.data,zName.len);
		if(zone!=NULL){
