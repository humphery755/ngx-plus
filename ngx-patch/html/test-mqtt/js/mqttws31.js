...
Messaging = (function (global) {
	...
	/** EXTEND RC Meaning. */
	var EXTEND_RC={
		1:"Connection Refused: identifier rejecte"
	};

	/** MQTT_EXTEND RC Meaning. */
	var MQTT_EXTEND_RC={
		1:"1001"
	};
    /**
    ...
    WireMessage.prototype.encode = function() {
    	...
    	switch(this.type) {
	        ...
	        case MESSAGE_TYPE.DISCONNECT:
	        	break;
			case MESSAGE_TYPE.EXTEND:
				remLength += 1;//SYNC PROTOCOL
				destinationNameLength = UTF8Length(this.payloadMessage.destinationName);
				remLength += destinationNameLength + 2;
...
//    	    	break;
			...
			case MESSAGE_TYPE.EXTEND:
				...
				pos = writeString(this.payloadMessage.destinationName, destinationNameLength, byteStream, pos);
				for (var i=0; i<this.offsets.length; i++) {
					pos = writeUint32(this.offsets[i].min,byteStream, pos);
					pos = writeUint32(this.offsets[i].max,byteStream, pos);
    	    	}
	        	break;
    ...
    function decodeMessage(input) {
    	...
    	switch(type) {
    	    ...
    	    case MESSAGE_TYPE.PUBLISH:     	    	
                ...
                // Extended protocol 4 bytes by offset
				var offset = readUint32(input, pos);
    		    pos += 4;
	...
	function writeUint32(input, buffer, offset) {
    	buffer[offset++] = input >> 24 & 0xff;      
    	buffer[offset++] = input >> 16 & 0xff; 
		buffer[offset++] = input >> 8 & 0xff;  
    ...
    ClientImpl.prototype.subscribe = function (filter, subscribeOptions) {
		...
		if (typeof filter == "string"){
			...
			for(var i=0;i<filter.length;i++){
				subscribeArray[filter[i]]=filter[i];				
				if (subscribeOptions.qos != undefined)
		...
		if (typeof filter == "string")subscribeArray[filter]=filter;
		else{
			for(var i=0;i<filter.length;i++){
				subscribeArray[filter[i]]=filter[i];
			}
	...
	ClientImpl.prototype.sync = function (message, subscribeOptions) {
    	this._trace("Client.subscribe", filter, subscribeOptions);
              
    	if (!this.connected)
    	    throw new Error(format(ERROR.INVALID_STATE, ["not connected"]));
    	
        var wireMessage = new WireMessage(MESSAGE_TYPE.EXTEND);
		wireMessage.extProtocol=RESERVED_TYPE.SYNC;
		wireMessage.topics = [message.destinationName];
        wireMessage.offsets=message.offsets;
    ...
    ClientImpl.prototype.unsubscribe = function(filter, unsubscribeOptions) {  
		...
		if (typeof filter == "string")wireMessage.topics = [filter];
		else
			wireMessage.topics = filter;
		...
		if (typeof filter == "string")subscribeArray[filter]=null;
		else{
			for(var i=0;i<filter.length;i++){
				subscribeArray[filter[i]]=null;
			}
			
		}
		subscribeArray[filter]=null;
    ...
    ClientImpl.prototype._on_socket_message = function (event) {
        ...
        switch(wireMessage.type) {
			...
			case MESSAGE_TYPE.PINGREQ:
            	/* The sendPinger or receivePinger may have sent a ping, the receivePinger has already been reset. */
            	console.log("PINGREQ from server");
				pingrespMessage = new WireMessage(MESSAGE_TYPE.PINGRESP);
                this._schedule_message(pingrespMessage);
            	break;
			...
			case MESSAGE_TYPE.EXTEND:
				isExtend = true;
				this._disconnected(MQTT_EXTEND_RC[wireMessage.returnCode],EXTEND_RC[wireMessage.returnCode]);
				break;
            default:
            	this._disconnected(ERROR.INVALID_MQTT_MESSAGE_TYPE.code , format(ERROR.INVALID_MQTT_MESSAGE_TYPE, [wireMessage.type]));
        }; 
    ...
    var Client = function (host, port, clientId) {
        ...
        this.connect = function (connectOptions) {
            ...
            if (connectOptions.keepAliveInterval === undefined)
            	connectOptions.keepAliveInterval = 5;
        ...
        this.subscribe = function (filter, subscribeOptions) {
        	...
        	validate(subscribeOptions,  {qos:"object", 
        		                         invocationContext:"object", 
        		                         onSuccess:"function", 
        		                         onFailure:"function",
        		                         timeout:"number"
        	...
        	if (typeof subscribeOptions.qos !== "undefined" ){
				for(var i=0;i<subscribeOptions.qos.length;i++){
					if(!(subscribeOptions.qos[i] === 0 || subscribeOptions.qos[i] === 1 || subscribeOptions.qos[i] === 2 ))
